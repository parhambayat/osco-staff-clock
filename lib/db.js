...
export async function setCafeLocationSetting(location) {
  ...
  const value = JSON.stringify({
    lat,
    lng,
    // Keep in sync with lib/geofence DEFAULT/MIN (800 / 600) — old 250 caused false “not in range”.
    radiusM: Number.isFinite(radiusM) && radiusM > 0 ? Math.min(Math.max(radiusM, 600), 3000) : 800,
