...
// ——— App settings (café Wi-Fi IPs, etc.) ———
...
export async function getCafeWifiIpsSetting() {
  if (isLocalDevMode()) {
    ...
    if (!row?.value) return { ips: null, source: 'env' };
    return { ips: String(row.value), source: 'db' };
  }
  ...
  if (error) {
    // Table may not exist yet — fall back to env defaults.
    console.warn('[app_settings]', error.message);
    return { ips: null, source: 'env', error: error.message };
  }
  if (!data?.value) return { ips: null, source: 'env' };
    ...
    return {
      error:
        error.message.includes('app_settings') || error.code === '42P01'
          ? 'Run supabase-app-settings.sql in Supabase SQL Editor once, then try again.'
          : error.message,
    };
  }
  return { ok: true, value, source: 'db' };
