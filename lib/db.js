...
import { isOpenShiftOverdue, overdueClockOutIso } from './time.js';
...
const LOCATION_EXEMPT_KEY = 'location_exempt_staff';
const LOCATION_EXEMPT_PENDING_ID = 'settings-location-exempt';
const LOCATION_EXEMPT_PENDING_PHONE = '__osco_location_exempt__';
...
function isSystemPendingRow(row) {
  ...
  return (
    ...
    normalizePhone(phone) === LOCATION_EXEMPT_PENDING_PHONE ||
...
function isLocationExemptPendingRow(row) {
  return (
    row?.id === LOCATION_EXEMPT_PENDING_ID ||
    row?.phone === LOCATION_EXEMPT_PENDING_PHONE ||
    normalizePhone(row?.phone) === LOCATION_EXEMPT_PENDING_PHONE
  );
}
...
export async function listPending() {
  ...
  await sb
    ...
    .neq('id', LOCATION_EXEMPT_PENDING_ID)
  ...
  const { data, error } = await sb
    ...
    .neq('phone', LOCATION_EXEMPT_PENDING_PHONE)
...
export async function listOpenShifts() {
  if (isLocalDevMode()) {
    return readTable('shifts').filter((s) => s.clock_out == null);
  }
  if (!hasDatabase()) return [];
  const sb = getSupabase();
  const { data, error } = await sb
...
/**
 * Close open shifts that passed the nightly 00:40 auto clock-out.
 * Returns closed shift rows. Safe to call on every punch/summary.
 */
export async function autoCloseOverdueOpenShifts({ staffId } = {}) {
  if (!hasDatabase() && !isLocalDevMode()) return { closed: [], error: dbMissing().error };

  let open;
  if (staffId) {
    const one = await getOpenShift(staffId);
    open = one ? [one] : [];
  } else {
    open = await listOpenShifts();
  }

  const closed = [];
  const now = new Date();
  for (const s of open) {
    if (!s || !isOpenShiftOverdue(s.clock_in, now)) continue;
    const clock_out = overdueClockOutIso(s.clock_in);
    if (isLocalDevMode()) {
      const rows = readTable('shifts');
      const idx = rows.findIndex((r) => r.id === s.id && r.clock_out == null);
      if (idx < 0) continue;
      rows[idx] = { ...rows[idx], clock_out };
      writeTable('shifts', rows);
      closed.push(rows[idx]);
      continue;
    }
    const sb = getSupabase();
    const { data, error } = await sb
      ...
      .maybeSingle();
    if (error) {
      console.error('[autoCloseOverdue]', s.id, error.message);
      continue;
    }
...
/** Café night shifts often end after midnight on the next calendar day. */
function normalizeShiftIso(clock_in, clock_out) {
  ...
  if (clock_out === undefined || clock_out === null || clock_out === '') {
    return { clock_in: inIso, clock_out: null };
  }

  let outMs = new Date(clock_out).getTime();
  if (Number.isNaN(outMs)) return { error: 'Invalid clock out time.' };

  // If out is same/earlier day (e.g. in 16:03, out 00:14), treat as next morning.
  let safety = 0;
  while (outMs <= inMs && safety < 2) {
    outMs += 24 * 60 * 60 * 1000;
    safety += 1;
  }
  if (outMs <= inMs) {
  ...
  return { clock_in: inIso, clock_out: new Date(outMs).toISOString() };
}

export async function createManualShift({ staffId, clock_in, clock_out }) {
  if (!hasDatabase() && !isLocalDevMode()) return dbMissing();

  const staff = await getStaffById(staffId);
  if (!staff) return { error: 'Staff not found.' };

  if (!clock_in) return { error: 'Clock in is required.' };
  const normalized = normalizeShiftIso(clock_in, clock_out);
  if (normalized.error) return { error: normalized.error };
  const { clock_in: inIso, clock_out: outIso } = normalized;

  if (outIso == null) {
    const existing = await getOpenShift(staff.id);
    if (existing) return { error: 'This staff already has an open shift. Close it or set a clock out.' };
  }
  ...
  if (isLocalDevMode()) {
    const rows = readTable('shifts');
    if (outIso == null && rows.some((s) => s.staff_id === staff.id && s.clock_out == null)) {
      return { error: 'This staff already has an open shift. Close it or set a clock out.' };
    }
    rows.unshift(shift);
    writeTable('shifts', rows);
    return { shift };
...
export async function updateShift(id, { clock_in, clock_out }) {
  ...
  if (isLocalDevMode()) {
    ...
    const normalized = normalizeShiftIso(
      clock_in !== undefined ? clock_in : current.clock_in,
      clock_out !== undefined ? clock_out : current.clock_out
    );
    if (normalized.error) return { error: normalized.error };
    const next = {
      ...current,
      clock_in: normalized.clock_in,
      clock_out: normalized.clock_out,
    };
...
function parseExemptStore(raw) {
  if (!raw) return { staffIds: [] };
  try {
    const obj = typeof raw === 'string' ? JSON.parse(raw) : raw;
    const staffIds = Array.isArray(obj?.staffIds)
      ? [...new Set(obj.staffIds.map((id) => String(id || '').trim()).filter(Boolean))]
      : [];
    return { staffIds };
  } catch {
    return { staffIds: [] };
...
async function readExemptStore() {
  const fromApp = await readAppSetting(LOCATION_EXEMPT_KEY);
  if (fromApp) return { store: parseExemptStore(fromApp), source: 'db' };

  const sb = !isLocalDevMode() && hasDatabase() ? getSupabase() : null;
  const pendingVal = await readPendingSetting(sb, {
    id: LOCATION_EXEMPT_PENDING_ID,
    phone: LOCATION_EXEMPT_PENDING_PHONE,
    isRow: isLocationExemptPendingRow,
  });
  if (pendingVal) return { store: parseExemptStore(pendingVal), source: 'pending' };
  return { store: { staffIds: [] }, source: 'none' };
}

async function writeExemptStore(store) {
  const value = JSON.stringify({
    staffIds: store.staffIds || [],
    updatedAt: new Date().toISOString(),
  });

  const app = await writeAppSetting(LOCATION_EXEMPT_KEY, value);
  if (app.ok) {
    const sb = !isLocalDevMode() && hasDatabase() ? getSupabase() : null;
    await writePendingSetting(sb, {
      id: LOCATION_EXEMPT_PENDING_ID,
...
export async function listLocationExemptStaffIds() {
  const { store } = await readExemptStore();
  return store.staffIds || [];
}

export async function isStaffLocationExempt(staffId) {
  const id = String(staffId || '').trim();
  if (!id) return false;
  const ids = await listLocationExemptStaffIds();
  return ids.includes(id);
}

export async function setStaffLocationExempt(staffId, enabled) {
  const staff = await getStaffById(staffId);
  if (!staff) return { error: 'Staff not found.' };

  const { store } = await readExemptStore();
  const set = new Set(store.staffIds || []);
  if (enabled) set.add(staff.id);
  else set.delete(staff.id);

  const saved = await writeExemptStore({ staffIds: [...set] });
  if (saved.error) return saved;
