...
export async function deleteStaff(id) {
  ...
  // Delete shifts first so it works even if FK cascade is missing in production.
  const { error: shiftErr } = await sb.from('shifts').delete().eq('staff_id', staffId);
...
/** Remove every shift for one staff member (keeps the person registered). */
export async function clearStaffShifts(staffId) {
  if (!hasDatabase() && !isLocalDevMode()) return dbMissing();

  const id = String(staffId || '').trim();
  if (!id) return { error: 'staffId required' };

  const staff = await getStaffById(id);
  if (!staff) return { error: 'Staff not found.' };

  if (isLocalDevMode()) {
    const before = readTable('shifts');
    const next = before.filter((s) => s.staff_id !== id);
    writeTable('shifts', next);
    return { ok: true, removed: before.length - next.length };
  }

  const sb = getSupabase();
  const { data, error } = await sb.from('shifts').delete().eq('staff_id', id).select('id');
  if (error) return { error: error.message };
...
export async function deleteShift(id) {
  ...
  if (!shiftId) return { error: 'id required' };
  ...
  const sb = getSupabase();
  const { data, error } = await sb.from('shifts').delete().eq('id', shiftId).select('id');
  if (error) return { error: error.message };
  if (!data?.length) return { error: 'Shift not found.' };
  return { ok: true };
}
