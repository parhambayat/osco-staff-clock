...
function isSystemPendingRow(row) {
  ...
  return (
    ...
    normalizePhone(phone) === CAFE_WIFI_PENDING_PHONE ||
...
export async function setCafeWifiIpsSetting(ipsCsv) {
  ...
  const app = await writeAppSetting(CAFE_WIFI_IPS_KEY, value);
  if (app.ok) {
    const sb = !isLocalDevMode() && hasDatabase() ? getSupabase() : null;
...
export async function setCafeLocationSetting(location) {
  ...
  const value = JSON.stringify({
    lat,
    lng,
    radiusM: Number.isFinite(radiusM) && radiusM > 0 ? radiusM : 250,
