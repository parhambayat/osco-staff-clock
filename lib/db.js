...
export async function createManualShift({ staffId, clock_in, clock_out }) {
  ...
  const inIso = new Date(clock_in).toISOString();
  if (Number.isNaN(new Date(inIso).getTime())) {
    return { error: 'Invalid clock in time.' };
  }

  let outIso = null;
  if (clock_out !== undefined && clock_out !== null && clock_out !== '') {
    outIso = new Date(clock_out).toISOString();
    if (Number.isNaN(new Date(outIso).getTime())) {
