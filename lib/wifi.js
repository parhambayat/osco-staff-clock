...
export const DEFAULT_CAFE_WIFI_IPS = [
  '37.40.228.35',
  '37.40.228.65',
  '37.40.224.218',
  '37.40.225.101',
...
/**
 * Env / build list, always merged with known café defaults so a single
 * stale Vercel dashboard CAFE_WIFI_IP cannot wipe the current modem IP.
 */
export function getAllowedCafeIpsFromEnv() {
  const raw = (process.env.CAFE_WIFI_IP || '').trim();
  return parseIpList([...DEFAULT_CAFE_WIFI_IPS, ...parseIpList(raw)]);
}
